...
* 全概率公式
  ...
  * $\Bbb E_{x\backsim p(x)}(f(x)) = \displaystyle\sum_x p(x)f(x)$
  * $\Bbb E$与$\sum$的互换
...
* Reward & Return
  ...
  * 回报 Return
    ...
    * 轨迹$\tau$的总回报: $G(\tau) = \sum^{T-1}_{t=0}{\gamma^tr_{t+1}}$
      * $G(\tau_{t_0})=\sum_{t_0}^{T-1}\gamma^{t-t_0}r_{t+1}$从$t_0$时刻开始计算的总回报
...
* 值函数
  * 状态值函数 State-Value Function: $V_{\pi_{\theta}}(s)$
    ...
    * $V_{\pi_{\theta}}(s) = \Bbb E_{\tau\backsim p(\tau)}[G(\tau_t)|\tau_{s_t} = s]$
